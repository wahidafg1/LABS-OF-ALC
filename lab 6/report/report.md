...
В логической рабочей области Cisco Packet Tracer была спроектирована и построена сеть, включающая маршрутизатор Cisco 2811 (msk-donskaya-aabdullakhi-gw-1), несколько коммутаторов Cisco 2960, оконечные устройства (персональные компьютеры) и серверы. Соединение между маршрутизатором и главным коммутатором `msk-donskaya-aabdullakhi-sw-1` организовано через интерфейс FastEthernet0/0, подключенный к порту 24 коммутатора. Данная линия сконфигурирована для передачи трафика всех VLAN. Остальные коммутаторы служат для подключения различных сегментов, включая рабочие станции и серверы web, file и mail.

![1.PNG-Построение топологии сети](1.PNG)
